<template>
  <aside class="w-14 bg-white border-r border-gray-200 flex flex-col items-center py-3 gap-1 z-30 shrink-0 relative">
    ...
    <div class="flex flex-col items-center gap-1 w-full px-2">
      <!-- Settings (WITH flyout submenu) -->
      <div class="relative w-full flex justify-center"
        @mouseenter="showSettingsMenu = true"
        @mouseleave="showSettingsMenu = false">
        <RouterLink to="/manage/user-management" custom v-slot="{ navigate, isActive }">
          <SidebarIcon :active="isActive || showSettingsMenu" title="Settings" @click="navigate">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
        ...
        <Transition name="flyout">
          <div
            ...
            class="absolute left-full bottom-0 z-50 w-60 bg-white border border-gray-200 shadow-lg rounded-r-md overflow-hidden max-h-[85vh] overflow-y-auto"
            @mouseenter="showSettingsMenu = true"
            @mouseleave="showSettingsMenu = false"
          >
            <div class="px-4 pt-4 pb-2">
              <p class="text-xl font-normal text-gray-400">Manage</p>
            </div>
            <nav>
              <RouterLink
...
import { manageMenuItems } from '@/data/settingsData.js'
